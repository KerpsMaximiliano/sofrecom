...
@Component({
    selector: 'app-license-view-delegate-edit',
    templateUrl: './license-view-delegate-edit.component.html'
  })

export class LicenseViewDelegateEditComponent implements OnInit, OnDestroy {
    ...
    save(): void {
        ...
        this.subscription = this.delegateService.save(item).subscribe(users => {
            ...
            this.router.navigate(['/rrhh/licenses/views/delegates']);
        },
        err => {
            this.messageService.closeLoading();
            this.errorHandlerService.handleErrors(err);
