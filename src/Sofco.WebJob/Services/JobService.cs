...
namespace Sofco.WebJob.Services
{
    public class JobService : IJobService
    {
        ...
        private void Init(string timeZoneName, List<Setting> settings)
        {
            ...
            RecurringJob.AddOrUpdate<IEmployeeWorkTimesAddJob>(JobNames.EmployeeWorkTimesAdd, j => j.Execute(), Cron.Weekly(DayOfWeek.Friday, 16, 57), localTimeZone);

            RecurringJob.AddOrUpdate<IEmployeeWorkTimesMonthAddJob>(JobNames.EmployeeWorkTimesMonthAdd, j => j.Execute(), Cron.Daily(23), localTimeZone);
